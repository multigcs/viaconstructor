...
from viaconstructor.xncparser import XncParser
...
def main() -> int:
    ...
    parser.add_argument("filename", help="xnc file", type=str)
    ...
    if not os.path.isfile(filename):
        ...
        sys.exit(1)
    xnc = open(filename, "r").read()
    xnc_parser = XncParser(xnc)
    minmax = xnc_parser.get_minmax()
    size = xnc_parser.get_size()
    ...
    if args.output:
        ...
        pygame.display.set_caption(f"xncpreview ({filename})")
